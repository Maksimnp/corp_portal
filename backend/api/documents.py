...
# DB_CONFIG = {
#     "host": os.getenv("DOCUMENTS_DB_HOST", "localhost"),
#     "db": os.getenv("DOCUMENTS_DB_DATABASE", "documents_db"),  # ← db вместо database
#     "user": os.getenv("DOCUMENTS_DB_USER", "minskhleb_portal_admin"),
#     "password": os.getenv("DOCUMENTS_DB_PASSWORD", "hLb&aafLejCc"),
#     "charset": "utf8mb4",  # Рекомендуется для поддержки Unicode
#     "autocommit": False,   # Важно! Будем управлять транзакциями вручную
# }
...
# # ← ЗАМЕНА: Функция подключения к MySQL
# async def get_db_connection():
#     # Добавьте эти строки для отладки
#     logger.info(f"DB_HOST: {os.getenv('DOCUMENTS_DB_HOST')}")
#     logger.info(f"DB_DATABASE: {os.getenv('DOCUMENTS_DB_DATABASE')}")
#     logger.info(f"DB_USER: {os.getenv('DOCUMENTS_DB_USER')}")
#     logger.info(f"DB_PASSWORD: {'*' * len(os.getenv('DOCUMENTS_DB_PASSWORD', '')) if os.getenv('DOCUMENTS_DB_PASSWORD') else 'NOT SET'}")
...
#     try:
#         # ← ЗАМЕНА: Используем %s вместо $1, $2 и DictCursor
...
#                 datetime.now(), datetime.now(), document.status.value, '')  # ← .value для Enum
#             )
#         await conn.commit()  # ← ВАЖНО: Фиксируем транзакцию
#         return document
#     except Exception as e:
#         await conn.rollback()  # ← ВАЖНО: Откатываем транзакцию при ошибке
...
#     try:
#         async with conn.cursor(aiomysql.DictCursor) as cursor:
#             # ← ЗАМЕНА: %s и DictCursor
#             await cursor.execute(
#                 "SELECT id, owner_username FROM documents WHERE id = %s",
#                 (document_id,)
#             )
...
#                 """,
#                 (document_id, recipient, shared_doc.permission.value, shared_doc.shared_at,
#                 shared_doc.status.value, title, user['username'], shared_doc.file_path, file_type)
#             )
#         await conn.commit()  # ← ВАЖНО: Фиксируем транзакцию
#         return shared_doc
#     except Exception as e:
...
#     try:
#         async with conn.cursor(aiomysql.DictCursor) as cursor:
#             query = "SELECT * FROM documents WHERE owner_username = %s"
#             params = [user['username']]
#             if search:
#                 query += " AND title LIKE %s"  # ← ЗАМЕНА: ILIKE -> LIKE
...
#                     status=DocumentStatus(doc['status']),  # ← Преобразование строки в Enum
...
#     try:
#         async with conn.cursor(aiomysql.DictCursor) as cursor:
#             query = "SELECT * FROM shared_documents WHERE recipient = %s"
#             params = [user['username']]
#             if search:
#                 query += " AND title LIKE %s"  # ← ЗАМЕНА: ILIKE -> LIKE
#                 params.append(f"%{search}%")
...
#             await cursor.execute(query, params)
#             shared_docs = await cursor.fetchall()

#             return [
#                 SharedDocument(
#                     document_id=str(doc['document_id']),
#                     recipient=doc['recipient'],
#                     permission=DocumentPermission(doc['permission']),  # ← Преобразование строки в Enum
...
#                     file_path=doc['file_path'],
#                     file_type=doc['file_type'],
#                     can_edit=doc['permission'] == DocumentPermission.EDIT.value  # ← Сравниваем со строкой
...
#         async with conn.cursor(aiomysql.DictCursor) as cursor:
#             await cursor.execute(
#                 "SELECT * FROM shared_documents WHERE document_id = %s AND recipient = %s",
#                 (document_id, user['username'])
#             )
#             shared_doc = await cursor.fetchone()
#             if not shared_doc:
#                 raise HTTPException(status_code=404, detail="Shared document not found")

#             await cursor.execute(
#                 "UPDATE shared_documents SET status = %s WHERE document_id = %s AND recipient = %s",
#                 (status.value, document_id, user['username'])  # ← .value для Enum
#             )
#         await conn.commit()  # ← ВАЖНО: Фиксируем транзакцию
#         return SharedDocument(
#             document_id=str(shared_doc['document_id']),
#             recipient=shared_doc['recipient'],
#             permission=DocumentPermission(shared_doc['permission']),  # ← Преобразование строки в Enum
...
#             can_edit=shared_doc['permission'] == DocumentPermission.EDIT.value  # ← Сравниваем со строкой
#         )
#     except Exception as e:
#         await conn.rollback()  # ← ВАЖНО: Откатываем транзакцию при ошибке
#         logger.error(f"Database error: {e}")
...
#         async with conn.cursor(aiomysql.DictCursor) as cursor:
#             await cursor.execute(
#                 "SELECT file_path, title, file_type FROM documents WHERE id = %s AND owner_username = %s",
#                 (docId, user['username'])
#             )
#             document = await cursor.fetchone()
#             if not document:
#                 await cursor.execute(
#                     "SELECT file_path, title, file_type FROM shared_documents WHERE document_id = %s AND recipient = %s",
#                     (docId, user['username'])
#                 )
#                 shared_doc = await cursor.fetchone()
#                 if not shared_doc:
...
#         async with conn.cursor(aiomysql.DictCursor) as cursor:
#             await cursor.execute(
#                 "SELECT file_path, owner_username FROM documents WHERE id = %s",
#                 (docId,)
#             )
#             document = await cursor.fetchone()
#             if not document:
...
#             await cursor.execute("DELETE FROM shared_documents WHERE document_id = %s", (docId,))
#             await cursor.execute("DELETE FROM documents WHERE id = %s", (docId,))
#         await conn.commit()  # ← ВАЖНО: Фиксируем транзакцию
...
# # Обработчики OnlyOffice остаются БЕЗ ИЗМЕНЕНИЙ, так как они не взаимодействуют с БД через asyncpg
...
#                     conn = await get_db_connection()
#                     try:
#                         async with conn.cursor(aiomysql.DictCursor) as cursor:  # ← ЗАМЕНА: DictCursor
#                             await cursor.execute(
...
#                     except Exception as e:  # ← ЗАМЕНА: Общее исключение
...
#                        WHERE d.id = %s AND (d.owner_username = %s OR sd.recipient = %s)""",
#                     (docId, user['username'], user['username'])
#                 )
#                 document = await cursor.fetchone()
#                 if not document:
...
#                 can_edit = is_owner or (document.get('permission') == DocumentPermission.EDIT.value) # ← Сравниваем со строкой
